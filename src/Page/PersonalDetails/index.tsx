...
const PersonalDetails = () => {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setErrors((currentErrors) => {
      const keys = Object.keys(currentErrors) as Array<
        keyof typeof currentErrors
      >;
      const errors = Object.fromEntries(
        keys.map((key) => [key, null])
      ) as typeof currentErrors;
      return errors;
    });
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        for (const key in errors) {
          validationErrors[key as keyof Inputs] = errors[key]?.at(0);
        }
...
type Errors = {
  [K in keyof Inputs]: string | null;
};

export default PersonalDetails;
export type { Inputs as PersonalDetailsInputs };
